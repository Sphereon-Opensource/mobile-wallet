...
import {
    ...
    _NormalizedVerificationMethod, extractPublicKeyHex,
    isDefined,
...
export const getFirstKeyWithRelation = async (identifier: IIdentifier, vmRelationship?: DIDDocumentSection, errorOnNotFound?: boolean): Promise<_ExtendedIKey | undefined> => {
 ...
 * @beta This API may change without a BREAKING CHANGE notice.
 */
export function extractPublicKeyHexWithJwkSupport(pk: _ExtendedVerificationMethod, convert: boolean = false): string {
...
export async function mapIdentifierKeysToDocWithJwkSupport(
    ...
    const documentKeys: VerificationMethod[] = await dereferenceDidKeysWithJwkSupport(
        didDocument,
        section,
        context,
    )
