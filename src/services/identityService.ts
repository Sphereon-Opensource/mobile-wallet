...
const fetchKeypair = async (): Promise<{publicKey: string; privateKey: string}> => {
  const response = await fetch('https://backend.hat.demo.sphereon.com/generate-keypair');
  if (!response.ok) {
    throw new Error(`Network response for generate-keypair was not ok. Error: ${response.statusText} ${await response.text}`);
  }
...
async function createRSAIdentifier(args: ICreateIdentifierArgs): Promise<IIdentifier> {
  try {
    /*  This also throws errors
        ...
        const privateKeyHex = privateKeyHexFromPEM(keyPair.private);
        const publicKeyHex = publicKeyHexFromPEM(keyPair.public);
    */
    /*
    ...
    */
    ...
    const {publicKey, privateKey} = await fetchKeypair();

    const keyType = 'RSA';
    const use = jwkDetermineUse(keyType, JwkKeyUse.Signature);
    const jwk: JsonWebKey = toJwk(publicKey, keyType, {use});
